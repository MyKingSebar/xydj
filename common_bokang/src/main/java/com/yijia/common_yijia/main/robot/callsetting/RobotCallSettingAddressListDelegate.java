...
public class RobotCallSettingAddressListDelegate extends LatteDelegate implements CommonStringClickListener,SwipeRefreshLayout.OnRefreshListener{
    RecyclerView mRecyelerView=null;
    String token = null;
    SwipeRefreshLayout mRefreshLayout = null;
    RobotCallSettingAddressListAdapter mAdapter=null;
        ...
        RxRestClient.builder()
                ...
                .subscribe(new BaseObserver<String>(getContext()) {
                    ...
                    public void onResponse(String response) {
                        LatteLogger.json("query_call_list", response);
                        final String status = JSON.parseObject(response).getString("status");
                        if (TextUtils.equals(status, "1001")) {
                            List<MultipleItemEntity> data = new RobotCallSettingAddressListDataConverter()
                                    .setJsonData(response)
                                    .convert();
                            if(data==null){
                                return;
                            }
                            mAdapter = new RobotCallSettingAddressListAdapter(data);
                            mAdapter.setmCommonClickListener(info -> getParentDelegate().getSupportDelegate().start(new InviteDelagate()));
                            mAdapter.setCommonClickLongStringListener((id, info) -> getParentDelegate().getSupportDelegate().start(new InviteDelagate()));
